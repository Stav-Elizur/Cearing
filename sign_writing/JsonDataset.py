...
class JsonlDataset(data.Dataset):
    def __init__(self, dir_path):
        ...
        with open('image_encodings.json') as f:
            ...
            self.image_encodings = list(filter(lambda file_name: os.path.isfile(os.path.join(self.dir_path, file_name["id"])),
                                               image_encodings))
    ...
    def __getitem__(self, index):
        ...
        image = Image.open(os.path.join(self.dir_path, data_dict["id"])).convert("RGB")
        image_processed = preprocess_transforms(image)
        image_tensor = image_processed
